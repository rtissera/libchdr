...
**It does not.** Splitting the three stages with per-stage timers, measured on
hardware:
...
compressed bytes captured from a failing run - also
...
Failure-path diagnostics in `zlib_codec_decompress()`, plus stage tags in
`cd_codec_decompress()` and `cdfl_codec_decompress()`, were what separated
the two independent bugs here; they were temporary and are not in the
tree. Note the earlier
