...
Measured with a dependent-chain probe (`idf.py -DBENCH_MULPROBE=1`):
...
`idf.py -DBENCH_MULPROBE=1 -DBENCH_ECCPROBE=1`. Cycles per 2352-byte sector, all
...
## Reproducing
    ...
    # op latency, allocator, ECC variants
    idf.py -DBENCH_MULPROBE=1 -DBENCH_ECCPROBE=1 build flash monitor
