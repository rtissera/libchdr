/*
 * RV32IMAFC/ilp32f RAM-budget probe for libchdr, run again for ESP32-P4.
 *
 * ESP32-P4's RISC-V cores share BL616's RV32IMAFC/ilp32f base ISA/ABI (see
 * cmake/toolchain-rv32imafc.cmake), so this is byte-identical to
 * tests/rv32/fw.c - a separate copy/workflow exists only so ESP32-P4 gets
 * its own named CI job and budget file, matching the rp2350-arm/
 * rp2350-riscv/rv32 pattern, not because the measured numbers differ.
 *
 ...
 * .github/workflows/esp32p4-ram-budget.yml for the full build+run recipe,
 * and tests/esp32p4/check_budget.py for how the printed numbers are
 * enforced.
 */
#include <stdio.h>
#include <stdlib.h>
...
int main(void)
{
	printf("=== libchdr rv32imafc/ilp32f (esp32-p4) peak-heap-per-codec ===\n");
